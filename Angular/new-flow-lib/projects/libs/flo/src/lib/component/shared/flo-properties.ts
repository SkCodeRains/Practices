...
export namespace Properties {
  ...
  export class DefaultCellPropertiesSource implements PropertiesSource {
    ...
    protected createProperty(metadata: Flo.PropertyMetadata | any): Property {
      return {
        id: metadata.id,
        name: metadata.name,
        type: metadata.type,
  ...
  export namespace Validators {
    ...
    export function uniqueResource(service: (value: any) => Observable<boolean>, debounceDuration: number): AsyncValidatorFn {
      return (control: AbstractControl): Observable<ValidationErrors> => {
        return new Observable<ValidationErrors>((obs: any) => {
          if (control.valueChanges) {
            ...
            service(control.value).subscribe({
              next: (res: any) => {
                if (res) {
                  obs.next(undefined);
                } else {
