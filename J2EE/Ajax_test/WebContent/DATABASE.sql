    DROP SCHEMA IF EXISTS soham;
	CREATE database IF not EXISTS soham;
	use soham;
	...
	DELIMITER //
	CREATE PROCEDURE searchAccount(Ano int)
	begin
	select * from accounts where accno=Ano;	
	END//
	CREATE PROCEDURE Accounts()
	begin
